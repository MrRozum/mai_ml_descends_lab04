...
class BaseDescent:
    ...
    def __init__(self, dimension: int, lambda_: float = 1e-3, loss_function: LossFunction = LossFunction.MSE):
        ...
        """
        self.w: np.ndarray = np.random.rand(dimension).reshape(-1 ,1)
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function is LossFunction.MSE:
            return np.sum((self.predict(x) - y.reshape(-1, 1)) ** 2) / x.shape[0]
...
class VanillaGradientDescent(BaseDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        """
        diff_weights = - self.lr() * gradient # in step() grad * lr
        self.w += diff_weights # update w
        return diff_weights
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        # np.sum((self.predict(x) - y) ** 2) / len(y)
        if self.loss_function is LossFunction.MSE:
            grad = (2 / x.shape[0]) * x.T @ (self.predict(x) - y.reshape(-1, 1)) # на 2 забиваем так как не влияет оптимизации
        elif self.loss_function is LossFunction.MAE:
            grad = x.T @ np.sign(self.predict(x) - y.reshape(-1, 1)) / x.shape[0]
        elif self.loss_function is LossFunction.LogCosh:
            grad = x.T @ np.tanh(self.predict(x) - y.reshape(-1, 1)) / x.shape[0]
        elif self.loss_function is LossFunction.Huber:
            error = self.predict(x) - y.reshape(-1, 1)
            mask = np.abs(error) <= delta
            grad = (1 / x.shape[0]) * x.T @ np.where(mask, 2 * error, delta*np.sign(error))
        return grad
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        indices = np.random.permutation(x.shape[0]) # shuffles data
        X_shuffled = x[indices]
        y_shuffled = y[indices]
        
        # Счетчики для усреднения 
        iter = 0
        summ_grad = np.zeros((x.shape[1], 1)) # с предположением, что bias уже сидит в матрице признаков x
        ...
        for i in range(0, X_shuffled.shape[0], self.batch_size):
            ...
            summ_grad += super().calc_gradient(X_batch, y_batch)
            iter += 1
            
        return summ_grad / iter
...
class MomentumDescent(VanillaGradientDescent):
    ...
    def __init__(self, dimension: int, lambda_: float = 1e-3, loss_function: LossFunction = LossFunction.MSE):
        ...
        self.h: np.ndarray = np.zeros(dimension).reshape(-1, 1)
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        # self.h = self.alpha * self.h + super().lr.__call__* super().calc_gradient(x, y)
...
class Adam(VanillaGradientDescent):
    ...
    def __init__(self, dimension: int, lambda_: float = 1e-3, loss_function: LossFunction = LossFunction.MSE):
        ...
        self.m: np.ndarray = np.zeros(dimension).reshape(-1, 1)
        self.v: np.ndarray = np.zeros(dimension).reshape(-1, 1)
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        diff_weights = -self.lr() * m_hat / (np.sqrt(v_hat) + self.eps)
        self.w += diff_weights
